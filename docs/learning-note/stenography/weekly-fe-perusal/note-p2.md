...
const user = useSelector((state) => userSelector(state), deepEqual);
```

> 这种方式适用于被缓存的函数具有幂等性，即入参一样，结果一样。 7. 结合外部变量的缓存查询
...
4. stage3 candidate，获取反馈，提供 polyfill 或 babel 插件
...
`try ... catch` 可以捕获执行上下文的错误，如果脱离上下文（如 setTimeout, 未开始执行时）则无法保证。

全局异常监控

- `window.addEventListener('error')`，所有运行时错误，包括同步和异步。不捕获语法、API、资源加载错误
- `window.addEventListener('unhandledrejection')`，由 Promise 抛出，但未被 catch 到的错误

## 21.精读《Web fonts: when you need them, when you don’t》

相比西文书写系统，中文书写系统使用 web font 的优劣：

1. 字体包更大
2. 特性字体（如 Logo），选择 png 或者 svg 方案更佳
3. 如使用动态生成字符字集，需要统计页面已使用的文字，花费不小

Web Font 的最大问题：

- FOUT（Flash of Unstyled Text），字体加载完成前，文本因字体切换导致的抖动
- FOIT（Flash of Invisible Text），字体加载完成前，文本不会显示

解决方案：

- font-display 属性，auto（FOIT），swap（FOUT），fallback 和 optional
- 预加载，css 的 @font-face 被读到时才开始加载，调用 link 的 rel=preload 提前加载
...
class Sample {
  ...
  static _ = initializer(this.a);
}

function initializer(a) {
  a.push(1);
}
...
画布应用语言，适合低代码、网站搭建、BI、Web Excel

## 212.精读《可维护性思考》

> 真实世界，我们用的每一件东西都是拷贝的，即使坏了也不会对其他相同物品产生任何影响
> 对于一个庞大而复杂的项目，不能指望每位开发者都了解全部原理后才能工作，我们需要在大多数时候把开发者当作消费者来看待，提供精美而稳定的接口。
> 提升代码质量本身就是一件缺乏动力去做的事，或者长期被认为是重要不紧急的事，往往很难找出理由现在就去做，但没有人觉得不应该做。
